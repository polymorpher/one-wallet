...
const WalletCard = ({ wallet }) => {
  const { address, name, effectiveTime, lifespan } = wallet
  ...
  return (
    ...
    >
      <Space direction='vertical' size='large'>
        ...
        <Text ellipsis={{ tooltip: address }} style={{ width: 196 }}>{address}</Text>
      </Space>
    </Card>
  )
}
...
const List = () => {
  ...
  const numWallets = Object.keys(wallets).length
  const noun = numWallets > 1 ? 'wallets' : 'wallet'
