...
contract ONEWallet {
    ...
    function revealTransfer(bytes32[] calldata neighbors, uint32 index, byte32 eotp, address dest, uint256 amount) external
    isValidIndex(index)
    ...
    function revealRecovery(bytes32[] calldata neighbors, uint32 index, byte32 eotp) external
    isValidIndex(index)
    isCorrectProof(neighbors, index, eotp)
    returns (bool)
    {
        bytes packed = bytes.concat(neighbors, bytes32(bytes4(index)), eotp);
    ...
    {
        uint32 counter = block.timestamp / interval - t0;
        require(counter == index, "Code has incorrect timestamp");
        _;
    }

    modifier isCorrectProof(bytes32[] calldata neighbors, uint32 index, byte32 eotp)
    {
        byte32 h = sha256(eotp);
