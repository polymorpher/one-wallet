...
const STATS_CACHE = process.env.OUT || './data/stats.json'
const ADDRESSES_OUT = process.env.OUT || './data/addresses.csv'
...
const search = async ({ address, target }) => {
  ...
  while (right < 0 || (left + 1 < right && left !== mid)) {
    ...
    const transactions = await api.rpc.getTransactionHistory({ address, pageIndex: mid, pageSize: 500, fullTx: false })
...
const scan = async ({ address, from = T0, to = Date.now(), updateBalance = false }) => {
  let pageIndex = await search({ address, target: to })
  let tMin = to
  console.log({ from, to })
  const wallets = []
  while (tMin > from) {
    const transactions = await api.rpc.getTransactionHistory({ address, pageIndex, pageSize: 500, fullTx: true })
    ...
    console.log(`searched up to time = ${timeString(tMin)}; at page ${pageIndex}; retrieved ${transactions.length} transactions from relayer; ${creations.length} creations of 1wallet`)
  ...
  const balances = updateBalance && await batchGetBalance(uniqueWallets.map(w => w.address))
  return { balances, wallets: uniqueWallets }
}

async function exec () {
  const fp = await fs.open(STATS_CACHE, 'a+')
  const fp2 = await fs.open(ADDRESSES_OUT, 'w+')
  ...
  for (const address of RELAYER_ADDRESSES) {
    const { balances, wallets } = await scan({ address, from, updateBalance })
