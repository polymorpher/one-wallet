...
library Guardians
{
     ...
     */
    function addGuardian(Core.Wallet storage _wallet, address _guardian) external {
        _wallet.info[_guardian].exists = true;
        _wallet.guardians.push(_guardian);
        _wallet.info[_guardian].index = uint128(_wallet.guardians.length - 1);
    }
     ...
     */
    function revokeGuardian(Core.Wallet storage _wallet, address _guardian) external {
     ...
     */
    function guardianCount(Core.Wallet storage _wallet) external view returns (uint256) {
     ...
     */
    function getGuardians(Core.Wallet storage _wallet) external view returns (address[] memory) {
     ...
     */
    function isGuardian(Core.Wallet storage _wallet, address _guardian) external view returns (bool) {
