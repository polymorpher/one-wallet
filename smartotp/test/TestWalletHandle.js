...
const Harmony = require('@harmony-js/core').Harmony
...
if (process.env['NETWORK'] === 'local') {
  hmy = new Harmony(
    ...
    {
      chainType: 'hmy',
      chainId: common.networkId.Testnet,
    },
  )
...
const gasPrice = common.gas.gasPrice
const gas = common.gas.gasLimit
...
contract('WalletHandle - TEST SUITE 1 [Initial checks and a transfer]', function (accounts) {
  ...
  it('Send money to Contract', function () {
    let smartContract
    return WalletHandle.deployed().then(function (instance) {
      ...
      return web3.eth.getBalance(smartContract.address)
      ...
      assert.equal(contrBalance, web3.utils.toWei('1', 'ether'))
...
contract('WalletHandle - TEST SUITE 2 [Deplete child tree OTPs, init new child tree ; deplete parent OTPs ; new parent tree]:', function (accounts) {
  const owner = accounts[0]
  const amount2Send = web3.utils.toWei('0.01', 'ether')
  const receiver = accounts[1]
  let smartContract
  it('Bootstrap / send 1 Eth at contract', async () => {
    ...
    smartContract = await WalletHandle.deployed()
    ...
    const tx = await hmy.blockchain.getTransactionByHash({ txnHash: txHash })
    ...
    const gasCost = BigInt(tx.result.gasPrice) * BigInt(txreceipt.gasUsed)

    const expectedBalance = BigInt(initialAmount)
    assert.equal(BigInt(await web3.eth.getBalance(smartContract.address)), expectedBalance)
  ...
  it('Try to initialize new child tree operation prematurely. ', async () => {
    const curChildTreeIdx = await smartContract.MT_parent_currentChildTreeIdx.call()
    const newChildTreeIdx = parseInt(curChildTreeIdx)
    const otpId = parseInt(await smartContract.getCurrentOtpID.call())

    try {
      await smartContract.adjustNewChildTree(
        auth.MT_child_depthOfCachedLayer, auth.getChildCachedLayer(newChildTreeIdx),
        ...auth.getAuthPath4ChildTree(newChildTreeIdx), ...auth.getConfirmMaterial(otpId), { from: owner }
      )
      assert.fail('Expected revert not received')
    } catch (error) {
      const revertFound = error.message.search('revert') >= 0
  ...
  it('Deplete child tree OTPs by TRANSFER operations', async () => {
    if (DO_TREE_DEPLETION) {
      let currentOtpID = parseInt(await smartContract.getCurrentOtpID.call())
      assert.equal(0, currentOtpID)

      const sizeOfChildTree = parseInt(await smartContract.MT_child_numberOfOTPs.call())

      for (let i = 0; i < sizeOfChildTree - 1; i += 1) {
        const tmpReceipt = await smartContract.initNewOper(receiver, amount2Send, OperationTypeEnum.TRANSFER, { from: owner })
        console.log(`\t \\/== Gas used in init TRANSFER[${i}] = `, tmpReceipt.receipt.gasUsed)

        // confirm operations as well
        const tmpReceipt2 = await smartContract.confirmOper(...auth.getConfirmMaterial(currentOtpID), currentOtpID, { from: owner })
        console.log(`\t \\/== Gas used in confirm TRANSFER[${i}] = `, tmpReceipt2.receipt.gasUsed)
        // Log is not provided on harmony somehow.
      ...
      try {
        await smartContract.initNewOper(receiver, amount2Send, OperationTypeEnum.TRANSFER, { from: owner })
        assert.fail('Expected revert not received')
      } catch (error) {
        const revertFound = error.message.search('revert') >= 0
        assert(revertFound, `Expected "revert", got ${error} instead`)

        const curTokID = await smartContract.getCurrentOtpID.call()
  ...
  it('Try to confirm OTP from invalid iteration layer. ', async () => {
    if (DO_TREE_DEPLETION) {
      try {
        const receipt = await smartContract.confirmOper(...auth.getConfirmMaterial(0), 0, { from: owner })
  ...
  it('Initialize a new child tree when child tree OTPs are depleted. ', async () => {
    if (DO_TREE_DEPLETION) {
      ...
      const curOTPID = await smartContract.getCurrentOtpID.call()
      ...
      const realNewChildTreeIdx = await smartContract.MT_parent_currentChildTreeIdx.call()
      assert.equal(parseInt(realNewChildTreeIdx), newChildTreeIdx)
      const newOTPID = await smartContract.getCurrentOtpID.call()
      assert.equal(parseInt(newOTPID), parseInt(curOTPID) + 1)

      const nopOper = await smartContract.pendingOpers.call(parseInt(curOTPID))
      assert(!nopOper[2]) // pending == false
      assert.equal(OperationTypeEnum.NOP, nopOper[3]) // operation type == NOP

      const newRoot = await smartContract.MT_child_rootHash.call()
  ...
  it('Try to init a new opearation when all parent OTPs are depleted.', async () => {
    if (DO_TREE_DEPLETION) {
      const currentOtpID = await smartContract.getCurrentOtpID.call()
      ...
      try {
        ...
        const currentOtpID2 = await smartContract.getCurrentOtpID.call()
  ...
  it('Adjust a new parent tree.', async () => {
    if (DO_TREE_DEPLETION) {
      let currentOtpID = await smartContract.getCurrentOtpID.call()
      assert.equal(auth.MT_parent_numberOfOTPs - 1, parseInt(currentOtpID))
      const oldChildRoot = await smartContract.MT_child_rootHash.call()
      const sizeOfstage1Buf = await smartContract.getSizeOfNPT_stage1Buf.call()
      const sizeOfstage2Buf = await smartContract.getSizeOfNPT_stage2Buf.call()
      ...
      await smartContract.adjustNewParentTree_stage1(maliciousHashOfRootAndOTP, { from: owner })
      await smartContract.adjustNewParentTree_stage1(hashOfRootAndOTP, { from: owner })

      // stage 2 - put root into 2nd stage buffer
      await smartContract.adjustNewParentTree_stage2(oldChildRoot, { from: owner }) // malicious
      await smartContract.adjustNewParentTree_stage2(oldChildRoot, { from: owner }) // malicious
      await smartContract.adjustNewParentTree_stage2(auth.MT_parent_rootHash, { from: owner }) // legitimate
      console.log('puting new parent root hash into buffer of stage 2 = ', auth.MT_parent_rootHash)

      // stage 3 - provide otp
      await smartContract.adjustNewParentTree_stage3(cmAndSides[0], cmAndSides[1],
        auth.MT_child_depthOfCachedLayer, auth.getChildCachedLayer(0), ...auth.getAuthPath4ChildTree(0)
      )
      // auth.dumpAllOTPs()
      // auth.dumpAllChildRootHashes()

      const newParentRoot = await smartContract.MT_parent_rootHash.call()
      assert.equal(newParentRoot, auth.MT_parent_rootHash)
      const newChildRoot = await smartContract.MT_child_rootHash.call()
      assert.equal(newChildRoot, auth.getAuthPath4ChildTree(0)[0][0])
      const curChildTreeIdx = await smartContract.MT_parent_currentChildTreeIdx.call()
      assert.equal(parseInt(curChildTreeIdx), 0)
      currentOtpID = await smartContract.getCurrentOtpID.call()
  ...
  const init3TransfersAndConfirm2nd = async (startOTPIdx) => {
    if (DO_TREE_DEPLETION) {
      ...
      const operation = await smartContract.pendingOpers.call(idxOfTx)
      assert(operation[2]) // is pending == true
      assert.equal(amount2Send, operation[1])

      const receipt = await smartContract.confirmOper(...auth.getConfirmMaterial(idxOfTx), idxOfTx, { from: owner })
      console.log(`\t \\/== Gas used in CONFIRM TRANSFER: `, receipt.receipt.gasUsed)

      const executedOper = await smartContract.pendingOpers.call(idxOfTx)
      console.log(executedOper)
      assert(!executedOper[2], `Operation must not be pending. Got: ${executedOper[2]}`) // pending == false
      const tx = await hmy.blockchain.getTransactionByHash({ txnHash: receipt.receipt.transactionHash })

      const gasCost = BigInt(BigInt(tx.result.gasPrice) * BigInt(receipt.receipt.gasUsed))
      console.log('\t \\/== Gas cost was: ', web3.utils.fromWei(gasCost.toString(), 'ether'), ' Eth')
      const balanceAfter = BigInt(await web3.eth.getBalance(smartContract.address))
  ...
  const depleteOTPs = async () => {
    if (DO_TREE_DEPLETION) {
      let childTreeIdx = await smartContract.MT_parent_currentChildTreeIdx.call()
      let currentOtpID = await smartContract.getCurrentOtpID.call()
      ...
      for (let t = parseInt(childTreeIdx); t < allChildTrees; t++) { // iterate over remaining child trees
        ...
        for (let i = parseInt(currentOtpID); i < ((t + 1) * auth._MT_child_numberOfOTPs) - 1; i += 1) {
          let tmpReceipt = await smartContract.initNewOper(receiver, amount2Send, OperationTypeEnum.TRANSFER, { from: owner })
          console.log(`\t \\/== Gas used in init TRANSFER[${i}] = `, tmpReceipt.receipt.gasUsed)
          console.log('[depleteOTPs] currentOtpID=', currentOtpID, 'i=', i)
          // confirm operations as well
          tmpReceipt = await smartContract.confirmOper(...auth.getConfirmMaterial(currentOtpID), currentOtpID, { from: owner })
        ...
        if (t < allChildTrees - 1) { // normal adjustment
          const tmpReceipt = await smartContract.adjustNewChildTree(auth.MT_child_depthOfCachedLayer, auth.getChildCachedLayer(t + 1),
          ...
          const newChildTreeIdx = await smartContract.MT_parent_currentChildTreeIdx.call()
          ...
          try {
            await smartContract.adjustNewChildTree(auth.MT_child_depthOfCachedLayer, auth.getChildCachedLayer(0),
        ...
        currentOtpID = await smartContract.getCurrentOtpID.call()
        childTreeIdx = await smartContract.MT_parent_currentChildTreeIdx.call()
      }
...
contract('WalletHandle - TEST SUITE 3 [Playing with daily limits]:', function (accounts) {
  ...
  it('Bootstrap / send 1 Eth at contract', async () => {
    ...
    let txreceipt = await web3.eth.sendTransaction({ from: sender,
      to: smartContract.address,
      value: initialAmount,
      gas,
      gasPrice })
    ...
    assert.equal(BigInt(await web3.eth.getBalance(smartContract.address)), expectedBallance)
  ...
  it('Try to send more than balance and generate 1 unconfirmed transfer', async () => {
    ...
    const tmpReceipt = await smartContract.initNewOper(receiver, tooBigAmount2Send, OperationTypeEnum.TRANSFER, { from: owner })
    console.log(`\t \\/== Gas used in init TRANSFER:`, tmpReceipt.receipt.gasUsed)
    ...
    try {
      const receipt1 = await smartContract.confirmOper(...auth.getConfirmMaterial(confirmedTokenId), confirmedTokenId, { from: owner })
  ...
  it('Adjust daily limit', async () => {
    ...
    const dailyLimBefore = await smartContract.dailyLimits.call()
    ...
    const tmpReceipt = await smartContract.initNewOper(NULL_ADDRESS, newDailyLimit, OperationTypeEnum.SET_DAILY_LIMIT, { from: owner })
    ...
    const dailyLimAfter = await smartContract.dailyLimits.call()
  ...
  it('Exceed the new daily limit and generate 1 unconfirmed transfer in buffer', async () => {
    ...
    const tmpReceipt = await smartContract.initNewOper(receiver, amountOutOfDailyLimit, OperationTypeEnum.TRANSFER, { from: owner })
    console.log(`\t \\/== Gas used in init TRANSFER:`, tmpReceipt.receipt.gasUsed)
    ...
    const currentOTPID3 = await smartContract.getCurrentOtpID.call()
  ...
  it('Increase daily limits and confirm the last transfer', async () => {
    const idxOfTx = await smartContract.getCurrentOtpID.call()
    const idxOfTxBefore = parseInt(idxOfTx) - 1
    const balanceBefore = BigInt(await web3.eth.getBalance(smartContract.address))

    const tmpReceipt = await smartContract.initNewOper(NULL_ADDRESS, newIncreasedDailyLimit, OperationTypeEnum.SET_DAILY_LIMIT, { from: owner })
    console.log(`\t \\/== Gas used in init SET_DAILY_LIMIT:`, tmpReceipt.receipt.gasUsed)

    const receipt1 = await smartContract.confirmOper(...auth.getConfirmMaterial(parseInt(idxOfTx)), parseInt(idxOfTx), { from: owner })
    console.log(`\t \\/== Gas used in confirm SET_DAILY_LIMIT:`, receipt1.receipt.gasUsed)

    const currentOtpID = await smartContract.getCurrentOtpID.call()
    assert.equal(parseInt(idxOfTx) + 1, parseInt(currentOtpID))

    const receipt2 = await smartContract.confirmOper(...auth.getConfirmMaterial(idxOfTxBefore), idxOfTxBefore, { from: owner })
  ...
  it('Exceed new daily limit by new accumulated transfer', async () => {
    ...
    const tmpReceipt = await smartContract.initNewOper(receiver, nextSmallAmount, OperationTypeEnum.TRANSFER, { from: owner })
    console.log(`\t \\/== Gas used in init TRANSFER:`, tmpReceipt.receipt.gasUsed)
    ...
    try {
      await smartContract.confirmOper(...auth.getConfirmMaterial(idxOfTx), idxOfTx, { from: owner })
      assert.fail('Expected revert not received')
    } catch (error) {
      const revertFound = error.message.search('revert') >= 0
      assert(revertFound, `Expected "revert", got ${error} instead`)
    ...
    const currentTokenIDAfter = await smartContract.getCurrentOtpID.call()
    assert.equal(parseInt(currentOTPID) + 1, parseInt(currentTokenIDAfter))
  })

  // Skipping this until evm_increaseTime and evm_decreaseTime are implemented on Harmony test RPC.
  it.skip('Confirm previous transfer in a next day - considering accumulated daily limit is reset', async () => {
    const idxOfTx = 4
    const balanceBefore = BigInt(await web3.eth.getBalance(smartContract.address))
    const currentOtpID = await smartContract.getCurrentOtpID.call()
    ...
    const receipt1 = await smartContract.confirmOper(...auth.getConfirmMaterial(idxOfTx), idxOfTx, { from: owner }) // date: Date.now()
    ...
    const operation2 = await smartContract.pendingOpers.call(idxOfTx)
...
contract('WalletHandle - TEST SUITE 4 [Playing with last resort stuff]:', function (accounts) {
  ...
  it('Bootstrap / send 1 Eth at contract', async () => {
    ...
    let txreceipt = await web3.eth.sendTransaction({ from: sender, to: smartContract.address, value: initialAmount, gas, gasPrice })
    // throw Error("...");
    const txHash = txreceipt.transactionHash
  ...
  it('Check last resort is off && receiver of last resort is account[5] && last active day is today', async () => {
    const lastResortInfo = await smartContract.lastResort.call()
  ...
  it('Check last resort is not set by calling send sendFundsToLastResortAddress (assuming deploy with timeout 0)', async () => {
    ...
    const tmpReceipt = await smartContract.sendFundsToLastResortAddress({ from: anybody })
  ...
  it('Change last resort timeout to 5 days', async () => {
    ...
    const tmpReceipt = await smartContract.initNewOper(NULL_ADDRESS, newTimeout, OperationTypeEnum.SET_LAST_RESORT_TIMEOUT, { from: owner })
    console.log(`\t \\/== Gas used in init SET_LAST_RESORT_TIMEOUT:`, tmpReceipt.receipt.gasUsed)

    // const currentOTPID2 = await contract.getCurrentOtpID.call()
    await smartContract.getCurrentOtpID.call()
    const operation = await smartContract.pendingOpers.call(idxOfOper)

    assert(operation[2]) // is pending == true
    assert.equal(newTimeout, operation[1])

    const tmpReceipt2 = await smartContract.confirmOper(...auth.getConfirmMaterial(idxOfOper), idxOfOper, { from: owner })
    ...
    const lastResortInfoAfter = await smartContract.lastResort.call()
  ...
  // Skipping this until evm_increaseTime and evm_decreaseTime are implemented on Harmony test RPC.
  it.skip('Try to invoke sendFundsToLastResortAddress after expired timeout', async () => {
  ...
  // Skipping this until evm_increaseTime and evm_decreaseTime are implemented on Harmony test RPC.
  it.skip('Test that contract was destroyed', async () => {
    try {
      const o = await smartContract.owner.call()
      console.log('owner = ', o)
...
contract('WalletHandle - TEST SUITE 5 [Further playing with last resort stuff - changing address, shifting time ]:', function (accounts) {
  ...
  const anybody = accounts[3]
  const receiverOfLastResortFunds = accounts[5]
  const newReceiverOfLastResortFunds = accounts[4]
  const initialFunds = BigInt(web3.utils.toWei('1', 'ether'))
  ...
  it('Bootstrap / send 1 Eth at contract', async () => {
    ...
    const gasCost = BigInt(tx.result.gasPrice) * BigInt(txreceipt2.gasUsed)
  ...
  it('Check last resort is off && receiver of last resort is receiverOfLastResortFunds && last active day is today', async () => {
    const lastResortInfo = await smartContract.lastResort.call()
    assert.equal(lastResortInfo[0], receiverOfLastResortFunds) // addr == account[5]
  ...
  it('Change last resort timeout to 5 days', async () => {
    ...
    const currentOtpID4 = await smartContract.getCurrentOtpID.call()
  ...
  it('Change last resort address to receiverOfLastResortFunds', async () => {
    const idxOfOper = 2
    const lastResortInfo = await smartContract.lastResort.call()
    ...
    const tmpReceipt = await smartContract.initNewOper(newReceiverOfLastResortFunds, 0, OperationTypeEnum.SET_LAST_RESORT_ADDRESS, { from: owner })
  ...
  // Skipping this until evm_increaseTime and evm_decreaseTime are implemented on Harmony test RPC.
  it.skip('Check last resort activity is updated after 4 days && sendFundsToLastResortAddress will not work in next 4 days', async () => {
    const idxOfOper = 3
    ...
    const tmpReceipt = await smartContract.initNewOper(NULL_ADDRESS, newDailyLimit.toString(), OperationTypeEnum.SET_DAILY_LIMIT, { from: owner })
    console.log(`\t \\/== Gas used in init SET_DAILY_LIMIT:`, tmpReceipt.receipt.gasUsed)
  ...
  // Skipping this until evm_increaseTime and evm_decreaseTime are implemented on Harmony test RPC.
  it.skip('Check sendFundsToLastResortAddress will work in next 1 day && contract was destroyed', async () => {
    await increaseTime(24 * 3600)
    const balanceOfReceiverBefore = BigInt(await web3.eth.getBalance(newReceiverOfLastResortFunds))
    await smartContract.sendFundsToLastResortAddress({ from: anybody })
...
contract('WalletHandle - TEST SUITE 6 [Token depletion + immediate send to last resort address]:', function (accounts) {
  ...
  it('Deplete tokens and measure gas consumption per a Transfer operation', async () => {
    ...
    for (let t = parseInt(childTreeIdx); t < allChildTrees; t++) { // iterate over remaining child trees
      // iterate over remaining child leaves
      for (let i = parseInt(currentOtpID); i < ((t + 1) * auth._MT_child_numberOfOTPs) - 1; i++) {
        const tmpReceiptInit = await smartContract.initNewOper(receiver, amount2Send, OperationTypeEnum.TRANSFER, { from: owner })
        const tmpReceiptConf = await smartContract.confirmOper(...auth.getConfirmMaterial(i), i, { from: owner })
      ...
      currentOtpID = await smartContract.getCurrentOtpID.call()
      if (t < allChildTrees - 1) { // normal adjustment
        const receiptChildTree = await smartContract.adjustNewChildTree(auth.MT_child_depthOfCachedLayer, auth.getChildCachedLayer(t + 1),
          ...auth.getAuthPath4ChildTree(t + 1), ...auth.getConfirmMaterial(parseInt(currentOtpID)), { from: owner }
        )
      ...
      currentOtpID = await smartContract.getCurrentOtpID.call()
      console.log(`currentOtpID: ${currentOtpID}`)
      childTreeIdx = await smartContract.MT_parent_currentChildTreeIdx.call()
    }
    assert.equal(parseInt(currentOtpID), auth.MT_parent_numberOfOTPs - 1)
    assert.equal(parseInt(childTreeIdx), allChildTrees - 1)
